...
class DarcyFlowSolution(object):
    ...
    def set_inflow_rate(self, flow_rate):
        t = self.time
        pressure_amplitude = 5000  # Pa
        pressure_frequency = 0.25  # Hz
        pressure_offset = 101325  # atmospheric pressure

        assert self.inlet_mask.sum()
        assert self.outlet_mask.sum()

        pressure = pressure_amplitude * sin(2 * math.pi * pressure_frequency * t) + pressure_offset

        # apply pressure boundary condition here
        self.set_pressure(pressure, self.inlet_mask)
    ...
    def test_inflow_outflow(self):
        ...
        tolerance = 1e-5
        assert abs(self.inflow + self.outflow) < tolerance
...
if __name__ == '__main__':
    it.command("program call 'particles.dat'")
